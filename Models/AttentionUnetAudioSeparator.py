        ...
        with tf.variable_scope("separator", reuse=reuse):
            ...
            for i in range(self.num_layers):
                ...
                if self.use_attention:
                    atten_U = tf.layers.conv1d(current_layer, self.atten_u, 1, activation=None)
                    atten_C = tf.layers.conv1d(enc_outputs[-i-1], self.atten_u, 1, activation=None)
                    bias_B = tf.Variable(tf.zeros([self.atten_u]))
                    # B = tf.keras.activations.sigmoid(tf.nn.bias_add(atten_U + atten_C, bias_B))
                    bias_A = tf.Variable(tf.zeros([1]))
                    # A = tf.keras.activations.sigmoid(tf.nn.bias_add(tf.layers.conv1d(B, 1, 1, activation=None), bias_A))
                    current_layer = Utils.crop_and_concat(tf.keras.layers.Multiply()([A, enc_outputs[-i-1]]), current_layer, match_feature_dim=False)
                else:
